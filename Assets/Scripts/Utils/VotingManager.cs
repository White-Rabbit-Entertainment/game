...
public class VotingManager : MonoBehaviour {
  ...
  List<PlayableCharacter> playersVotingFor;
  List<PlayableCharacter> playersVotingAgainst;
  public PlayableCharacter suspectedPlayer;
  PlayableCharacter voteLeader;

  public GameObject faceTrackerObj;

  [SerializeField] private TextMeshProUGUI faceExpression;

  public void Update() {
    // Check if the vote has run out of time, if so end the vote
    if (voteStarted) {
      voteTimeRemaining.text = $"{(int)Timer.voteTimer.TimeRemaining()}s";
      faceExpression.text = $"{suspectedPlayer.Owner.NickName} is looking {suspectedPlayer.lastEmotion}";
      if (Timer.voteTimer.IsComplete()) {
        EndVote();
      }
    }
  ...
  public void EndVote() {
    if (NetworkManager.instance.GetMe() == suspectedPlayer){
        faceTrackerObj.SetActive(false);
      }
    setVoteUI.SetActive(false);
    currentVoteUI.SetActive(false);
    voteTopRightUI.SetActive(false);
