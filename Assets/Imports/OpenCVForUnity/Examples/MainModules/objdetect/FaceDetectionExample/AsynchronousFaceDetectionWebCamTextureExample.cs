...
namespace OpenCVForUnityExample
{
    ...
    public class AsynchronousFaceDetectionWebCamTextureExample : MonoBehaviour
    {
        ...
        string emotion_classifier_filepath = "Assets/StreamingAssets/face-recognition.onnx";
        ...
        void Update ()
        {
            if (webCamTextureToMatHelper.IsPlaying () && webCamTextureToMatHelper.DidUpdateThisFrame ()) {
                ...
                for (int i = 0; i < detectedObjectsInRegions.Count; i++) {
                    Size inpSize = new Size(48, 48);
                    Mat cropped = new Mat(grayMat, rects[0]);

                    Mat blob = Dnn.blobFromImage(cropped, 1.0f, inpSize);
                    Mat res = new Mat();
                    // Mat blobConv = new Mat();
                    ...
                    // Debug.Log(max);
                    if (emotions[maxIndex] != "Angry") Debug.Log(emotions[maxIndex]);
